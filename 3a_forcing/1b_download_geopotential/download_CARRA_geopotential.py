...
def download_carra_geopotential(year, month, output_file):
...
def subset_iceland_geopotential(input_file, output_file):
    ...
    try:
        ...
        with Dataset(output_file, 'w', format='NETCDF4') as nc:
            ...
            geopotential = nc.createVariable('geopotential', 'f4', ('time', 'point'), zlib=True, complevel=4)
            ...
            with open(input_file, 'rb') as f_in:
                while True:
                    ...
                    geopotential[time_index, :] = masked_values

                    eccodes.codes_release(msg)

            print(f"Final dimensions - time: {nc.dimensions['time'].size}, point: {nc.dimensions['point'].size}")
            print(f"Variable geopotential shape: {nc.variables['geopotential'].shape}")
...
def main():
    ...
    if carra_folder == 'default':
        carra_folder = make_default_path('forcing/0_geopotential')
    else:
    ...
    for year in years:
        for month in months:
            raw_file = carra_folder / f'carra_geopotential_raw_{year}_{month}.grib'
            subset_file = carra_folder / f'carra_geopotential_iceland_{year}_{month}.nc'
            ...
            if raw_file.exists():
                ...
                if not download_carra_geopotential(year, month, raw_file):
                    print(f"Failed to download data for {year}-{month}, skipping.")
                    continue

            # Proceed with subsetting
            if subset_iceland_geopotential(raw_file, subset_file):
