...
# Counts the occurrence of each soil class in each HRU using rasterstats.
...
try:
    ...
    with rasterio.open(raster_path) as src:
        ...
        soil_data = src.read(1)  # Assuming the data is in the first band
    ...
    # Rename columns to match the desired format with integer USGS classes
    result_df = result_df.rename(columns=rename_column)

    # Ensure all values are integers
    for col in result_df.columns:
        if col != 'count':  # Assuming 'count' is not a soil class column
