...
basins_path = root_path / f'domain_{full_domain_name}' / 'shapefiles' / 'river_basins' / 'Yukon_Merit_Basins_fixed.shp'
rivers_path = root_path / f'domain_{full_domain_name}' / 'shapefiles' / 'river_network' / 'Yukon_RiverNetwork_MERIT.shp'
...
# Convert HRU_area to float64 and scale if necessary
subset_basins['HRU_area'] = subset_basins['HRU_area'].astype('float64')
subset_basins['HRU_area'] = subset_basins['HRU_area'] / 1_000_000  # Convert to square kilometers
subset_basins['HRU_area'] = subset_basins['HRU_area'].round(6)

# Define the output file paths for the subset domain
output_basins_path = make_default_path(controlFolder, controlFile, f'shapefiles/river_basins/{domain_name}_basins.shp')
output_rivers_path = make_default_path(controlFolder, controlFile, f'shapefiles/river_network/{domain_name}_network.shp')
