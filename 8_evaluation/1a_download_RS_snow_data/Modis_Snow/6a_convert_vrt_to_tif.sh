...
dest_line=$(grep -m 1 "^observation_snow_vrt4_path" ../../../0_control_files/control_active.txt) # full settings line
...
do
    ...
    # source path
    source_path="${root_path}/domain_${domain_name}/observations/RS_Snow/MOD10A1/5_multiband_domain_vrt_epsg_4326/$YEAR"

    fi

    # --- Location where converted data needs to go
    dest_line=$(grep -m 1 "^observation_snow_tif_path" ../../../0_control_files/control_active.txt) # full settings line
    ...
    # destination path
    dest_path="${root_path}/domain_${domain_name}/observations/RS_Snow/MOD10A1/6_tif_multiband/$YEAR"
    fi
    ...
    # Create .tif file
    #---------------------------------
    gdal_translate -co "COMPRESS=DEFLATE" $vrt_file $tif_file -co BIGTIFF=YES
...
# Make the log
this_file='6a_convert_vrt_to_tif.sh'
