...
if [ "$source_path" = "default" ]; then
 ...
 # source path
 source_path="${root_path}/domain_${domain_name}/observations/RS_Snow/MOD10A1/3_vrt_epsg_4326"

fi

# --- Location where cropped VRT needs to go
dest_line=$(grep -m 1 "^observation_snow_vrt3_path" ../../../0_control_files/control_active.txt) # full settings line
...
if [ "$dest_path" = "default" ]; then
 ...
 # destination path
 dest_path="${root_path}/domain_${domain_name}/observations/RS_Snow/MOD10A1/4_domain_vrt_epsg_4326"
fi
echo $dest_path
...
do
	...
	do
		...
		FILE_DES=$dest_path/$YEAR/"domain_"$FILENAME
...
# Make the log
this_file='4a_specify_subdomain.sh'
